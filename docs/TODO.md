...
- 記事内のリンクを OGP として表示させるようにする
  - リンクの OGP を取得する
    - open graph scrapper を使う
    - 記事の中にリンクが入っていたら都度変換して良い感じのスタイルに置き換える
      - amazon と同じタイミングで
  - ベタおきのリンクか、既に変換されたリンクかどうかを判別する方法を考える
...
remark-rehype と rehype-stringfy は HTML を書いても消えてしまう

https://zenn.dev/yodaka/articles/cee5943b92dfed

Git でファイル名の大文字・小文字の変更を検知するようにする - Qiita
